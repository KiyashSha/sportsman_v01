...
def login_user(request):
    if request.user.is_authenticated:
        return render(request, 'students/login_form.html',{'error_message':'Already logged in as: %s'%request.user.username})
    else:
...
def students_all(request):
    if not request.user.is_authenticated:
        ...
        students_all = Student.objects.all().order_by('student_classcode')
        ...
        return render(request, 'students/profile_all.html',{'students': students})
...
def bulk_upload(request):
    if not request.user.is_authenticated:
        ...
        if request.method == 'POST' and 'submit_file' in request.POST:
            ...
            fs = FileSystemStorage(location=os.path.join(settings.MEDIA_URL,'uploads'),base_url=os.path.join(settings.MEDIA_URL,'uploads'))
...
def create_student(request):
    ...
    return render(request, 'students/student_create_form.html', context)
...
def update_student(request, student_admno):
    if not request.user.is_authenticated:
        ...
        return render(request,'students/student_update_form.html',{'form':form})
